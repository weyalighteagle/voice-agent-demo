  ...
  openaiWs.on("message", async (data) => {
    ...
    try {
      ...
      if (msg.type === "conversation.item.input_audio_transcription.completed") {
        ...
        // ── Guard 1: Short transcript = just wake word = likely hallucination
        if (wakeWordEnabled && transcript.trim().length > 0 && transcript.trim().length < 15) {
          if (containsWakeWord(transcript, wakeWord)) {
            // Strip "hey", the wake word itself, and punctuation — is anything left?
            const stripped = transcript.toLowerCase()
              .replace(/hey/gi, "")
              .replace(new RegExp(wakeWord.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "gi"), "")
              .replace(/[.,!?\s]/g, "")
              .trim();
            if (stripped.length < 3) {
              console.log(`[relay] HALLUCINATION BLOCKED — short wake-word-only transcript: "${transcript}"`);
        ...
        if (wakeWordEnabled && !isAwake) {
          if (containsWakeWord(transcript, wakeWord)) {
            console.log(`[relay] ★ WAKE WORD DETECTED — activating for: "${transcript}"`);
            isAwake = true;
